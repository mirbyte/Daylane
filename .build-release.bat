...
if errorlevel 1 goto :failed
move /y "%OUT%\self-contained\Daylane.exe" "%OUT%\self-contained\Daylane-x64-self-contained.exe" >nul
if errorlevel 1 goto :failed

echo.
echo [2/2] Framework-dependent (requires .NET 10 Desktop Runtime)...
msbuild Daylane.csproj %COMMON% /p:SelfContained=false /p:PublishSingleFile=true /p:IncludeNativeLibrariesForSelfExtract=true /p:PublishDir=%OUT%\framework-dependent\
if errorlevel 1 goto :failed
move /y "%OUT%\framework-dependent\Daylane.exe" "%OUT%\framework-dependent\Daylane-x64.exe" >nul
if errorlevel 1 goto :failed

echo.
echo Done:
echo   %OUT%\self-contained\Daylane-x64-self-contained.exe
echo   %OUT%\framework-dependent\Daylane-x64.exe
goto :end
